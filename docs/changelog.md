...
### [3.3.1 (2026-01-25)](https://github.com/ihabunek/twitch-dl/releases/tag/3.3.1)

* Fix fetching chapters (#188)

### [3.3.0 (2025-12-15)](https://github.com/ihabunek/twitch-dl/releases/tag/3.3.0)

* Better support for HD streams (1440p, 2160p), these should now work without
...
* **Requires Python 3.9+**
* Experimental command `twitch-dl chat` has been split into separte subcommands
  `twitch-dl chat video` and `twitch-dl chat json`
* Add `twitch-dl chat ytt` for generating chat as subtitles which can be
  uploaded to youtube
...
* Add ability to download sub-only videos without authentication (thanks to
  @Haehnchen)
* **BREAKING**: Default video format has been changed from mkv to mp4, since
  that's a more common format.
* **BREAKING**: When running `twitch-dl info --json`, the JSON format for
  playlists has changed.
* Fix invalid filenames on windows when using {datetime} or {time} placeholders.

### [2.11.0 (2025-03-03)](https://github.com/ihabunek/twitch-dl/releases/tag/2.11.0)
...
* Add info --auth-token option to be able to show info for a sub-only video
...
### [2.9.3 (2025-01-04)](https://github.com/ihabunek/twitch-dl/releases/tag/2.9.3)

* IMPORTANT: Fix issue where the folder passed to `--cache-dir` when downloading
  videos would be deleted (#168)
* Fix encoding issue on windows where unicode characters in filename could break
  video download (#170)
